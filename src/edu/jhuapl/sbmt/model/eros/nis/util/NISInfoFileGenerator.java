...
public class NISInfoFileGenerator
{
	...
	static
	{
		try
		{
			...
			while (scanner.hasNextLine() && !found)
			{
				...
				String y = tokens[2];
				String z = tokens[3];
				nisFileToSunPositionMap.put(file,
						new Vector3D(Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)).normalize());
			}
			scanner.close();
		}
		catch (IOException e)
	...
	public NISInfoFileGenerator(String sourceDirectory, String destinationDirectory, String inputList,
			SmallBodyModel smallBodyModel, SpectraHierarchicalSearchSpecification searchSpec) throws IOException
	{
		...
		while (line != null && !line.equals(""))
		{
			...
			System.out.println(
					"NISInfoFileGenerator: NISInfoFileGenerator: spectrumFile " + spectrumFile.getAbsolutePath());

			NISSpectrum spectrum = new NISSpectrum(spectrumFile.getAbsolutePath(),
					(SpectrumInstrumentMetadataIO) searchSpec, smallBodyModel.getBoundingBoxDiagonalLength(),
					SpectrumInstrumentFactory.getInstrumentForName("NIS"));
			...
			File outputFile = new File(subDir,
					File.separator + FilenameUtils.getBaseName(spectrumFile.getAbsolutePath()) + ".INFO");

			System.out
					.println("NISInfoFileGenerator: NISInfoFileGenerator: output file " + outputFile.getAbsolutePath());
			...
			writer.newLine();
			writer.write("SPACECRAFT_POSITION = ( " + spacecraftPosition[0] + " , " + spacecraftPosition[1] + " , "
					+ spacecraftPosition[2] + " )");
			writer.newLine();
			writer.write("BORESIGHT_DIRECTION = ( " + frustumCenter[0] + " , " + frustumCenter[1] + " , "
					+ frustumCenter[2] + " )");
			writer.newLine();
			...
			writer.newLine();
			writer.write("SUN_POSITION_LT     = ( " + toSunUnitVector[0] + " , " + toSunUnitVector[1] + " , "
					+ toSunUnitVector[2] + " )");
...
//
//		SmallBodyViewConfig config = SmallBodyViewConfig.getSmallBodyConfig(ShapeModelBody.EROS,
...
//
//		NISInfoFileGenerator generator = new NISInfoFileGenerator(sourceDirectory.getAbsolutePath(),
//				destinationDirectory.getAbsolutePath(), "nisTimes.txt", smallBodyModel);
	}
